...
type CaloriesCalculator interface {
	// Calories добавьте необходимые методы в интерфейс
	Calories() float64
...
func (r Running) Calories() float64 {
	// вставьте ваш код ниже
	durationInMin := r.Duration.Hours() * MinInHours
	return (CaloriesMeanSpeedMultiplier*r.meanSpeed() + CaloriesMeanSpeedShift) * r.Weight / MInKm * durationInMin
}
...
func (w Walking) Calories() float64 {
	...
	speedInMSec := math.Pow(w.meanSpeed()*KmHInMsec, 2)
	heightInM := w.Height / CmInM
	durationInMin := w.Duration.Hours() * MinInHours
	return (CaloriesWeightMultiplier*w.Weight + (speedInMSec/heightInM)*CaloriesSpeedHeightMultiplier*w.Weight) * durationInMin
}
